...
void makeUpper(int debugFlag, int inFileDesc, int outFileDesc){
  ...
  while( ( feedback = system_call(3, inFileDesc, c, 1) ) > 0){    
    ...
    feedback = system_call(SYS_WRITE,outFileDesc,c, 1);/*sys_write*/
    if(debugFlag == 1 && c[0] != '\n'){
...
void flame2(int debugFlag, int pFlag, char prefix[]){
  ...
  while(count>0 && i < count){
	if(pFlag == 1 && strncmp(entp->buf, prefix, strlen(prefix)) != 0 ){
		i = i + entp->len;
