...
void updateProcessList(process **process_list){
	process* listHead = process_list[0];
...
void removeProcessFromList(process** process_list, int pid){
	...
	if(current==NULL){}//list has 1 node
	while(current!=NULL){
		if(current->pid==pid){
...
void addToDeleteList(process* proce){
	proce->next = list_to_delete;
	list_to_delete = proce;
...
void printProcessList(process** process_list){
	...
	while(1){
		switch(listHead->status){
			...
			case TERMINATED:
				...
				//removeProcessFromList(process_list,listHead->pid);//freshly terminated
				break;
			case SUSPENDED:
	...
	if (current!=NULL && current->status == TERMINATED){
		proces_list = current->next;
		//freeCmdLines(current->cmd);
		//free(current);
		addToDeleteList(current);
		current = proces_list;
		prev = current;
	}
...
{
	int pid = 0; //for suspend, kill and wake
	if (debugFlag == 1)
	...
	if (strcmp(pCmdLine->arguments[0], "suspend") == 0){
		...
		kill(pid, SIGTSTP);
		updateProcessStatus(proces_list, pid, SUSPENDED);
		return;
	}
		...
		pid = atoi(pCmdLine->arguments[1]);
		kill(pid, SIGCONT);
		updateProcessStatus(proces_list, pid, RUNNING);
		return;
	}
