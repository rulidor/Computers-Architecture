...
void printRelocation(){
        ...
        if (shdr[i].sh_type == SHT_REL) {
            ...
            for (int k = 0; k < header->e_shnum; k++){
                if (shdr[k].sh_type == SHT_SYMTAB || shdr[k].sh_type == SHT_DYNSYM) {
                    symTable = (Elf32_Sym *)((char *)map_start + shdr[k].sh_offset);
                    symbolNum = shdr[k].sh_size / shdr[k].sh_entsize;
                    linkToStrTable=shdr[k].sh_link;
                    symStrTab_p = map_start + shdr[linkToStrTable].sh_offset; //points to string table of symbol table
                    printf("<offset> <info> <type> <value> <name>\n");
                    for(int j=0; j<entryNum; j++){
                        printf("%x\t %x\t %x\t", relocationTable[j].r_offset, relocationTable[j].r_info, ELF32_R_TYPE(relocationTable[j].r_info));
                        int symTableIndex= ELF32_R_SYM(relocationTable[j].r_info); //index in the symTable
                        printf("%x\t %s", symTable[symTableIndex].st_value, symStrTab_p + symTable[symTableIndex].st_name);
                        printf("\n");
                    }
    ...
    Elf32_Shdr *shdr2 = (Elf32_Shdr *)(map_start + header->e_shoff); //points to sections
