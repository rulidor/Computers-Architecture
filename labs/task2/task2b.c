...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
	printf("******Activating kill_virus function*******\n");
	FILE * stream = fopen(fileName, "r+");
	fseek(stream, signitureOffset, SEEK_SET);
	char* myNOP = "\x90";
	int i;
	for(i=0; i<signitureSize; i++){
		fseek(stream, signitureOffset+i, SEEK_SET);
		fwrite(myNOP, 1, 1, stream);
	}

	fclose(stream);
	printf("******kill_virus function has done*******\n");
...
struct fun_desc menu[] = { {"Load signatures" , NULL}, { "Print signatures", NULL}, { "Detect viruses", NULL},{ "Fix file", NULL},{ "Quit", NULL}, { NULL, NULL } };
char userIn[30];
int signitureOffset;
int signitureSize;
char inPath[30];
...
do {
	...
	switch(choice){
		...
		case 2:
			...
			buffer = malloc(sizeof(char)*buffLength);

			fread(buffer, 1, buffLength, streamIn);

			
			detect_virus(buffer, buffLength, list);
			free(buffer);
			fclose(streamIn);
		...
		case 3:
			printf("Please enter file name to kill a virus:\n");
			fgets (fileName, 30, stdin);
			sscanf(fileName, "%s", fileName);
			printf("fileName: %s\n", fileName);

			printf("Please enter signiture offset:\n");
			fgets (userIn, 30, stdin);
			signitureOffset = atoi(userIn);
			printf("Please enter signiture size:\n");
			fgets (userIn, 30, stdin);
			signitureSize = atoi(userIn);
